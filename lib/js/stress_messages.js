...
stress.Message.prototype = {
	create: function(scene, type, text) {
		...
		if (type === 'gameOver') {
			...
			var gameOverText = this.scene.game.add.text(stress.game.width / 2, stress.game.height / 2 - 80, 'Opgeven is geen optie, je kan het! \nProbeer het nog een keer in je eigen tempo!', normalStyle);
		...
		if (type === 'gameOver' || type === 'completed') {
			// score
			var levelScore = this.scene.game.add.text(stress.game.width / 2, stress.game.height / 2 + 40, this.scene.score + ' Balans Punten', style);
