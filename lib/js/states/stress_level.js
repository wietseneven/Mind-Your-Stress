...
stress.Level.prototype = {
	...
	create: function () {
		...
		//  The Player
		if (this.thisLevel.customDude) {
			this.player = this.add.sprite(64, 0, this.levelName() + '_' + localStorage.gender + '_dude');
		} else {
		...
		var style = { font: "bold 32px Carter One", fill: "#fff", boundsAlignH: "center", boundsAlignV: "middle" };
		this.score = 0;
		...
		if (this.thisLevel.messages) {
			this.thisLevel.messages(this);
		}
		...
		this.finishLine = this.game.add.image(this.world.width - 400, 280, 'finish-line');
		this.finishLine.scale.setTo(0.4,0.4);
	},
	stopGame: function() {
	...
	increaseMultiplier: function() {
		...
		//this.score = 400;
		var newScore = Math.floor(((this.scores.t0 * 2) + (this.scores.t1 * 8) + (this.scores.t2 * 16) + (this.scores.t3 * 32) + (this.scores.t4 * 64)) * 1.4);
		this.score = newScore;
		this.displayScore = newScore + ' ' + multiplierLevel;
	...
	update: function () {
		...
		if (stress.global.gameOver || stress.global.notPlaying) {
			...
			this.levelScore.setText(this.displayScore);
