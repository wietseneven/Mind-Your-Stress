...
stress.Level.prototype = {
	...
	create: function () {
		this.background = this.add.tileSprite(0, 0, stress.dim.w(), stress.dim.h(), global.sceneName() + '-background');
		this.background.fixedToCamera = true;

		this.skyline = this.add.tileSprite(0, 0, stress.dim.w(), 407, global.sceneName() + '-skyline');
		this.skyline.fixedToCamera = true;

		//  Platforms that don't move
		this.stationary = this.add.physicsGroup();
		this.stationary.create(0, stress.dim.h() - 60, global.sceneName() + '-platform');
		this.stationary.setAll('body.allowGravity', false);
		this.stationary.setAll('body.immovable', true);


		//  The Player
		this.player = this.add.sprite(64, 0, global.sceneName() + '-dude');
