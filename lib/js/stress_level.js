...
stress.Level = function () {
	...
	this.levels = [level_home];
	this.levelName = function () {
		return this.levels[stress.global.level].name;
	};
...
stress.Level.prototype = {
	...
	init: function () {
		...
		this.world.resize(stress.dim.w() * 10, stress.dim.h());
	...
	create: function () {
		...
		this.skyline = this.add.tileSprite(0, 0, stress.dim.w(), 407, this.levelName() + '-skyline');
		this.skyline.fixedToCamera = true;

		//  Platforms that don't move
		this.stationary = this.add.physicsGroup();
		this.stationary.create(0, stress.dim.h() - 60, this.levelName() + '-platform');
		...
		// Background music
		this.music = this.game.add.audio('irresistible', 0.5, true, true);
		this.music.stop();
		this.music.play();
	},
	restart: function () {
	...
	adjustMusic: function () {
		this.music._sound.playbackRate.value = stress.global.speed / 1000;
	},
	preRender: function () {
