...
level_bedroom = {
	...
	images: function(level) {
		...
		// level.thoughLife = level.add.sprite(250, level.game.height - 480, 'level_notify-thoughlife');
		// level.thoughLife.scale.setTo(0.3,0.3);
		// level.thoughLife.fixedToCamera = true;
		// level.thoughLife.alpha = 0;
	},
	messages: function (level) {
		level.introtext = level.add.sprite(200, level.game.height - 430, 'bedroom-intro');
	...
	laundryBin: function(level, x) {
		var bin = level.add.sprite(x, stress.game.height - level.platformTop - 110, 'bedroom-laundryBin');
		bin.scale.setTo(0.25, 0.25);
